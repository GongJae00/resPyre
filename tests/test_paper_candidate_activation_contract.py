...
def test_paper_candidate_activation_contract_fails_closed_when_windowed_inputs_missing() -> None:
...
def test_paper_candidate_activation_contract_accepts_active_final_path() -> None:
    _require_paper_candidate_activation(
        video="trial_a",
        cfg=_paper_cfg(),
        state_window_rows=[{"group": "G_OF", "start_sec": 0.0, "end_sec": 8.0, "score": 0.8}],
...
def test_paper_candidate_activation_contract_fails_when_state_role_rows_missing() -> None:
...
def test_paper_candidate_activation_contract_allows_readout_only_scope_without_state_rows() -> None:
    cfg = {**_paper_cfg(), "reliability_prior_scope": "readout_only"}
    _require_paper_candidate_activation(
        video="trial_a",
        cfg=cfg,
        state_window_rows=[],
