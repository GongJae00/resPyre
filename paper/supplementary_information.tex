...
\subsection*{Label-free reliability law}
The reliability evidence used by PARH-OSSM is computed without using the
respiratory reference labels. Each candidate observation contributes
label-free features describing rate agreement, preprocessing invariance,
event timing, bridge timing, rate/phase support, morphology support, ROI
observability, harmonic ambiguity, nuisance evidence, and abstention pressure.
...
invariance, $T$ timing reliability, $E$ event timing, $U$ nuisance evidence,
...
\toprule
Dataset & Role & Label scope & Main use & Supported claim \\
\midrule
\endfirsthead
\toprule
Dataset & Role & Label scope & Main use & Supported claim \\
...
\caption{Complete eight-observation rate diagnostic plot. The figure compares direct observations, OSSM-KF, and observation-class-local PARH timing behavior across the fixed observation classes and is used only as diagnostic context, not as a post hoc best-source selector.}
\end{figure}

This figure expands the rate evidence beyond the three compact rows in the main
text. It shows whether fixed observations, OSSM-KF, and PARH-style readouts fail
for the same reason or for different observation-class reasons. As diagnostic
context, it explains the observed boundary without defining a post hoc
best-source policy.
...
observability and nuisance, but it is not used to replace the
...
This figure checks whether operating-point behaviour is stable within and across
evaluation settings. Its role is to document readout-policy boundaries, not to
introduce an additional tuned model variant.
